...
ob_start();
    echo "<table border='2px solid black;'><tr><td colspan='2' align='center'><b>0 BACKLOGS</b></td></tr>\n";
    echo "<tr><td><b>Roll No</b></td><td align='center'><b>Name</b></td></tr>\n";
    ...
    header("Content-Disposition: attachment; filename=\"nonbacklog.xls\"");
...
<center>
            ...
            <input type="submit" name="export" value="Export"></center>
        </form>
    </div>
    <br>
    <script>
...
if (isset($_POST['submit'])) {
    ...
    foreach ($htno as $rollno) {
        ...
        if ($flag == 0) {
            ...
            $row = $r->fetch_assoc();
            $name = $row["Name"];
             $exportData[] = array($rollno, $name);
            $table .= "<tr><td>{$rollno}</td><td>{$name}</td></tr>";
        }


    }
    $_SESSION['exportData'] = $exportData;
    $table .= "</table>";
    $tabledata = $table;
    echo $table;
}
