...
<body>
  ...
  <div id="mySidenav" class="sidenav">
    ...
    <center>
        <h2 style='color:blue;margin-top:-1%;'>REMOVE TEACHER</h2>
    </center><br><br>
    <form action="" method="post" enctype="multipart/form-data">
        <input type="email" name="mail" placeholder="Email" value="<?php echo $_POST['mail'] ?? ''; ?>" required />
        <center>
...
if (isset($_POST['submit'])) {
    ...
    if ($result) {
        ...
        if ($tableExists) {
            $query = "SELECT * FROM {$tablename} WHERE `dotnet_mail`='{$mail}'";
            $res = mysqli_query($conn, $query);
            if (mysqli_num_rows($res) > 0) {
                $query = "DELETE FROM {$tablename} WHERE `dotnet_mail`='{$mail}'";
                $res = mysqli_query($conn, $query);
                if($res){
                    exit("<div id='success'>Successfully deleted.</div>");
                }
                else{
                    exit("<div id='wrong'>Error in deleting data.</div>");
                }
            }
            else{
